<%-- 
    Document   : adminPanel
    Created on : 02.02.2021, 10:22:20
    Author     : Comp
--%>
            ...
            <p>
                ...
                <select name="userId" class="form-select">
                    ...
                    <c:forEach var="entry" items="${usersMap}">
                        <option value="${entry.key.id}" <c:if test="${userId == entry.key.id}">selected</c:if>>${entry.key.buyer.firstname} ${entry.key.buyer.lastname}, логин: ${entry.key.login}, роль:  ${entry.value}</option>
                    </c:forEach>
                </select>
            </p>
            <p>
